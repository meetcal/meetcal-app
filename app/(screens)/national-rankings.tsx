...
import { fetchNationalRankings, NationalRanking } from '@/lib/database/fetch-national-rankings';
...
export default function NationalRankingsScreen() {
  ...
  const [expandedSection, setExpandedSection] = useState<'gender' | 'ageGroup' | 'weightClass' | null>(null);
  ...
  return (
    <ThemedView style={[styles.container, { backgroundColor: colors.background }]}> 
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        <View style={[styles.card, { backgroundColor: colors.card }]}> 
          <View style={[styles.headerRow, { borderBottomColor: colors.border }]}> 
            <ThemedText style={[styles.headerCell, { width: 50, color: colors.secondaryText }]}>Rank</ThemedText>
            <ThemedText style={[styles.headerCell, { flex: 1, color: colors.secondaryText }]}>Name</ThemedText>
            <ThemedText style={[styles.headerCell, { width: 80, color: colors.secondaryText }]}>Total</ThemedText>
          </View>
          ...
          {loading ? (
            ...
            rows.map((athlete, index) => (
              ...
              >
                <ThemedText style={[styles.cell, { width: 50, color: colors.text }]}> 
                  {index + 1}
                </ThemedText>
                <ThemedText style={[styles.cell, { flex: 1, color: colors.text }]} numberOfLines={1}> 
                  {athlete.name}
                </ThemedText>
                <ThemedText style={[styles.cell, { width: 80, color: colors.text }]}> 
                  {Math.round(athlete.total)}kg
                </ThemedText>
              </View>
            ))
          )}
      ...
      >
        ...
        >
          <View style={[styles.modalContent, { backgroundColor: colors.card, maxHeight: windowHeight * 0.8 }]}> 
              ...
              <View style={styles.modalScrollContent}>
                <ScrollView bounces={false}>
                      ...
                      <ScrollView style={[styles.filterOptions, { borderTopColor: colors.border }]} bounces={false}>
                        {(['Men', 'Women'] as Gender[]).map((gender) => (
                          <Pressable
                            ...
                            onPress={() => {
                              setTempFilters(prev => {
                                const nextClasses = getWeightClasses(gender, prev.ageGroup);
                                return {
                                  ...prev,
                                  gender,
                                  weightClass: nextClasses.length > 0 ? nextClasses[0] : prev.weightClass,
                                };
                              });
                              setExpandedSection(null);
                            }}
                          >
                            <ThemedText style={[styles.filterOptionText, { color: colors.text }, tempFilters.gender === gender && { color: colors.link }]}>
                              {gender}
                            </ThemedText>
                            {tempFilters.gender === gender && <IconSymbol name="checkmark" size={16} color={colors.link} />}
                          </Pressable>
                        ))}
                      </ScrollView>
                    )}
                    ...
                    >
                      <View style={styles.filterSectionButtonContent}>
                        <View>
                          ...
                          <ThemedText style={[styles.filterSectionValue, { color: colors.text }]}>{tempFilters.ageGroup}</ThemedText>
                        </View>
                      ...
                      <ScrollView style={[styles.filterOptions, { borderTopColor: colors.border, maxHeight: maxOptionsHeight }]} bounces={false}>
                        {AGE_GROUPS.map((age) => (
                          <Pressable
                            ...
                            onPress={() => {
                              const nextClasses = getWeightClasses(tempFilters.gender, age);
                              setTempFilters(prev => ({
                                ...prev,
                                ageGroup: age,
                                weightClass: nextClasses.length > 0 ? nextClasses[0] : prev.weightClass,
                          ...
                          >
                            <ThemedText style={[styles.filterOptionText, { color: colors.text }, tempFilters.ageGroup === age && { color: colors.link }]}>
                              {age}
                            </ThemedText>
                            {tempFilters.ageGroup === age && <IconSymbol name="checkmark" size={16} color={colors.link} />}
                          </Pressable>
                        ))}
                      </ScrollView>
                    )}
                  ...
                  <View style={styles.filterSection}>
                    <Pressable
                      style={({ pressed }) => [styles.filterSectionButton, pressed && { opacity: 0.8 }]}
                      onPress={() => setExpandedSection(expandedSection === 'weightClass' ? null : 'weightClass')}
                    >
                      <View style={styles.filterSectionButtonContent}>
                        <View>
                          <ThemedText style={[styles.filterSectionLabel, { color: colors.secondaryText }]}>Weight Class</ThemedText>
                          <ThemedText style={[styles.filterSectionValue, { color: colors.text }]}>{tempFilters.weightClass}</ThemedText>
                        </View>
                        <IconSymbol name={expandedSection === 'weightClass' ? 'chevron.down' : 'chevron.right'} size={16} color={colors.secondaryText} />
                      </View>
                    </Pressable>
                    {expandedSection === 'weightClass' && (
                      <ScrollView style={[styles.filterOptions, { borderTopColor: colors.border, maxHeight: maxOptionsHeight }]} bounces={false}>
                        {getWeightClasses(tempFilters.gender, tempFilters.ageGroup).map((weightClass) => (
                          <Pressable
                            key={weightClass}
                            style={({ pressed }) => [
                              styles.filterOption,
                              { borderBottomColor: colors.border },
                              tempFilters.weightClass === weightClass && { backgroundColor: colors.pressed },
                              pressed && { opacity: 0.8 }
                            ]}
                            onPress={() => {
                              setTempFilters(prev => ({ ...prev, weightClass }));
                              setExpandedSection(null);
                            }}
                          >
                            <ThemedText style={[styles.filterOptionText, { color: colors.text }, tempFilters.weightClass === weightClass && { color: colors.link }]}>
                              {weightClass}
                            </ThemedText>
                            {tempFilters.weightClass === weightClass && <IconSymbol name="checkmark" size={16} color={colors.link} />}
                          </Pressable>
                        ))}
                      </ScrollView>
                    )}
