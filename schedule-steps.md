...
- ✅ For sessions with varied start times:
  ...
  - ⬜️ Create calendar events with correct platform-specific times
  - ✅ Display individual platform times under each platform in session card
...
4. ✅ Modify Display Components
   ...
   - ✅ Add platform-specific times as subtitles in session cards:
     - ✅ Only show if platforms have different times
     - ✅ Display under each platform name
     - ✅ Keep consistent time format

5. ⬜️ Update Calendar Integration
   - ⬜️ Modify calendar event creation to use platform-specific times
   - ⬜️ Update in saved sessions view
   - ⬜️ Update in start list view
   - ⬜️ Update in schedule details view
...
6. Testing
   ...
   - Test sessions with varied times:
     ...
     - Verify platform subtitles in session cards
   - Test edge cases:
     - AM/PM handling
     - Midnight/noon cases
...
- ✅ Platform-specific times should only appear when times differ within a session
