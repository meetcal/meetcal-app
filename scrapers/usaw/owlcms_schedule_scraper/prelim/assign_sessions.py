...
def athlete_weight_matches(
    athlete_weight: str, session_weight_range: str, session_gender: Optional[str] = None
) -> bool:
    """
    ...
    session_weights_only = re.sub(
        r"^[WM]\d+(\s*-\s*[WM]\d+)?\s+", "", session_weight_range
    )
    ...
    weight_classes_in_session = [
        normalize_weight_token(w) for w in weight_classes_in_session
    ]
...
def find_matching_session(athlete: Dict, schedule: List[Dict]) -> Optional[Dict]:
    ...
    for session in schedule:
        ...
        # Check weight class
        weight_category = session.get("age_group_weight_category", "") or session.get(
            "weight_category", ""
        )
    ...
    if weight_matched_sessions and athlete_total > 0:
        ...
        for session in weight_matched_sessions:
            ...
            if min_total <= athlete_total <= max_total:
                ...
                distance = min(
                    abs(athlete_total - min_total), abs(athlete_total - max_total)
                )
...
def assign_sessions(start_list_file: str, schedule_file: str, output_file: str):
    ...
    for athlete in athletes:
        ...
        if matching_session:
            ...
            # Track counts
            session_key = (
                matching_session.get("sess", ""),
                matching_session.get("plat", ""),
            )
            ...
            unassigned_count += 1
            print(
                f"  WARNING: Could not assign {athlete['name']} ({athlete['gender']}, {athlete['age']}, {athlete['weight_class']}, {athlete['entry_total']}, {athlete.get('meet', '')})"
            )
...
def main():
    ...
    parser.add_argument(
        "--start-list", default="vwf_umwf_start_list.csv", help="Path to start list CSV"
    )
    parser.add_argument(
        "--schedule", default="full_output.csv", help="Path to schedule CSV"
    )
    parser.add_argument(
        "--output", default="assigned_athletes.csv", help="Output CSV filename"
    )
